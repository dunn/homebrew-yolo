class Ayo < Formula
  ...
  homepage "https://github.com/ayojs/ayo"
  head "https://github.com/ayojs/ayo.git"
  ...
  conflicts_with "node", :because => "Ayo installs a `node` compatibility symlink"
  ...
  def install
    ...
    if build.with? "npm"
      # Allow npm to find Ayo before installation has completed.
  ...
  test do
    ...
    output = shell_output("#{bin}/ayo #{path}").strip
    assert_equal "hello", output
    output = shell_output("#{bin}/ayo -e 'console.log(new Intl.NumberFormat(\"en-EN\").format(1234.56))'").strip
    assert_equal "1,234.56", output
    if build.with? "icu4c"
      output = shell_output("#{bin}/ayo -e 'console.log(new Intl.NumberFormat(\"de-DE\").format(1234.56))'").strip
      assert_equal "1.234,56", output
    end

    if build.with? "npm"
      # make sure npm can find ayo
      ENV.prepend_path "PATH", opt_bin
      ENV.delete "NVM_NODEJS_ORG_MIRROR"
      assert_equal which("ayo"), opt_bin/"ayo"
